...
const usePoliceMissing = () => {
  const { updateBalance } = useGame();
  const activeFile = GameState.getActiveVictim();
  const [db, setDb] = useState(GameState.getCitizens());
  const [deceasedDb, setDeceasedDb] = useState(GameState.getDeceasedCitizens());
  const { checkSectionComplete } = useVictimFilePanel();
  const missingPersons = useMemo(() => {
    const livingMissing = db.filter((c) => c.policeStatus === "ZAGINIONY");
    const deadMissing = deceasedDb.filter((c) => !c.bodyFound);
    return [...livingMissing, ...deadMissing];
  }, [db, deceasedDb]);
  ...
  const handleSendData = (citizen) => {
    if (activeFile?.pesel === citizen.pesel && isAllSegmentsComplete()) {
      ...
      GameState.transferToInvestigation(citizen.id, new Date().toISOString().replace('T', ' ').substring(0, 16));
